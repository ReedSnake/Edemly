...
public sealed class NoteRefactorTests
{
    ...
    public async Task Controller_GetById_Should_Return_Forbid_When_Service_Returns_Forbidden()
    {
        ...
        service.Setup(x => x.GetById(0, 42))
            .ReturnsAsync(ServiceDataResult<NoteDto>.Forbidden());
    ...
    [Test]
    public async Task Controller_GetCount_Should_Map_Service_Data_To_Count_Object()
    {
        var service = new Mock<INoteService>();
        service.Setup(x => x.GetCount(0))
            .ReturnsAsync(ServiceDataResult<int>.Ok(7));
    ...
    [Test]
    public async Task Service_GetById_Should_Return_Forbidden_When_Note_Belongs_To_Other_User()
    {
        ...
        var result = await service.GetById(requester.Id, serverDb.Notes.Single().Id);
