...
The gateway routes `/main` and `/call` to the hub upstream, `/api/payment` to the payment upstream, and `/api` plus `/uploads` to the API upstream. The local `client.json` uses `http://localhost:3700` as a dedicated hub endpoint while API/payment requests use `3500` with `3600` as fallback. `GET /health` is proxied to the backend; `GET /gateway/health` checks only the gateway process.
...
5. Check the Velopack update feed after the main window is shown.
...
Generated release files in `deployment/local/static/updates/windows/stable` are ignored by Git; the repository keeps only `.gitkeep` for the directory.

Optional updates appear in the top status bar of the main window with `Update now`, `Remind later`, and `X`. Closing or postponing hides the bar only until the next client launch. Mandatory updates are controlled by `deployment/local/static/client.json` through `updates.mandatory` or `updates.minimumRequiredVersion`; when mandatory, the client starts installing the update without user confirmation.

## Client Releases

Release instructions live in [Client Releases and Updates](docs/server/RELEASES.md). The short version is:

1. Update `Edemly.Client/Edemly.Client.csproj` `<Version>`.
2. Update `deployment/local/static/client.json` `updates.latestVersion`, `updates.minimumRequiredVersion`, and `updates.mandatory`.
3. Publish the Windows client so the main executable is `Edemly.exe`.
4. Pack Velopack artifacts into `deployment/local/static/updates/windows/stable`.
5. Start the static site or full Docker stack and install from `http://localhost:8080/download/`.
